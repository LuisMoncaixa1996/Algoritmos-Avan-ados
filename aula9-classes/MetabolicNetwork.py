...
class MetabolicNetwork (MyGraph):
    ...
    def convert_metabolite_net(self, gmr):
        for m in gmr.node_types["metabolite"]: #IDENTIFICAÇÃO DOS METABOLITOS
            self.add_vertex(m) #ADICIONAR NÓDULO
            sucs = gmr.get_successors(m) #VERIFICAR QUAIS OS SUCESSORES DESSE NÓDULO
            for s in sucs:
                sucs_r = gmr.get_successors(s) #VERIFICAR OS SUC
                for s2 in sucs_r:
                    if m != s2:
                        self.add_edge(m, s2)
    ...
    def convert_reaction_graph(self, gmr): 
        for r in gmr.node_types["reaction"]: #identificação dos nodes na reação
            self.add_vertex(r) #adicionar node
            sucs = gmr.get_successors(r) #verificar quais os seus sucessores
            for s in sucs:
                sucs_r = gmr.get_successors(s) #verificar os sucessores dos sucessores obtidos
                for s2 in sucs_r:
                    if r != s2: self.add_edge(r, s2) #adiconar edge daqueles não existentes
